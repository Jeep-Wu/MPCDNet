...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
                          ...
                          epochs: int = 1000,
                          barrier: Barrier|None = None) -> nnUNetTrainer:
    ...
    nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold,
                                    ...
                                    device=device, epochs=epochs, barrier=barrier)
...
def run_ddp(rank, dataset_name_or_id, configuration, fold, tr, p, use_compressed, disable_checkpointing, c, val,
            pretrained_weights, npz, val_with_best, world_size, epochs:int=1000, barrier: Barrier = None):
    setup_ddp(rank, world_size)
    torch.cuda.set_device(torch.device('cuda', dist.get_rank()))

    nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, tr, p,
                                           use_compressed, epochs, barrier=barrier)
...
def run_training_entry(fold=None, barrier=None):
    import argparse
    ...
    parser.add_argument('--fold', type=str, required=False, default=fold,
    ...
    parser.add_argument('-epochs', type=int, default=1000, required=False,
                        help='[OPTIONAL] Specify the number of epochs to train for. Default: 1000')
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
                 ...
                 device=device, epochs=args.epochs, barrier=barrier)
