...
class MPCDNetPredictor(nnUNetPredictor):
    def __init__(self,
                 ...
                 args: dict = None):
        self.adjust = getattr(args, 'adjust', True)
        self.save_raw = getattr(args, 'save_raw', False)
        ori_output_folder = getattr(args, 'o', './outputs')
        self.pred_raw_folder = f'{ori_output_folder}/raw'
        ...
        os.makedirs(self.pred_raw_folder, exist_ok=True)
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                _pred = self.network(torch.flip(x, axes))
                pred += torch.flip(_pred, axes)
            stack_size = len(axes_combinations) + 1
            pred /= (stack_size)

        if self.adjust:
            pred[:,[1],...] = pred[:,[1],...] + pred[:,[2],...] - pred[:,[3],...]
            return pred
        else:
            return pred
    ...
    def predict_from_files_sequential(self,
        ...
        for li, of, sps in zip(list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files):
            ...
            self.ofile_name = os.path.basename(of)
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,
        ...
        self.ofile_name = os.path.basename(output_file_truncated)
        self.data_properties = dct['data_properties']
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        # for the fn fp
        dataset_json['labels'].update({'fn': 2, 'fp': 3})
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> torch.Tensor:
        ...
        for params in self.list_of_parameters:
            ...
            if prediction is None:
                ...
                _prediction = self.predict_sliding_window_return_logits(data).to('cpu')
